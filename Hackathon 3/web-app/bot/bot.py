...
class MessengerBot():
    ...
    # There should be a default method that will filter out any messages
    # that falls into a certain category

    def __init__(self, access_token, **kwargs):
        self.access_token = access_token
        self.secret_key = kwargs.get('secret_key')
        self.api_version = kwargs.get('api_version') or 7.0
    ...
    def auth_args(self):
        if not hasattr(self, '_auth_args'):
            ...
            if self.secret_key is not None:
                appsecret_proof = generate_appsecret_proof(self.access_token, self.secret_key)
                auth['appsecret_proof'] = appsecret_proof
            self._auth_args = auth
        return self._auth_args

    def __send_payload(self, payload):
        endpoint = self.graph_url + "/me/messages?access_token=" + self.access_token
    ...
    def send_recipient(self, recipient_id, payload, notification_type=NotificationType.regular):
        ...
        return self.__send_payload(payload)
    ...
    def send_quick_reply(self, recipient_id, text, quick_reply, notification_type=NotificationType.regular):
        payload = {
            "recipient": {
                "id": recipient_id 
            },
